"""chau_duckdb_retail.py - Project script (example).
...
def main() -> None:
    ...
    try:
        ...
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "chau_civic_event_clean.sql")

        # ----------------------------------------------------
        # STEP 2: BOOTSTRAP (create tables, load CSV data)
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "chau_civic_event_bootstrap.sql")

        # ----------------------------------------------------
        # STEP 3: RUN BASIC QUERIES
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "chau_civic_event_query_attendance_count.sql")
        run_sql_query(con, SQL_DIR / "chau_civic_event_query_civic_event_count.sql")
        run_sql_query(con, SQL_DIR / "chau_civic_event_query_attendance_by_event.sql")
        run_sql_query(con, SQL_DIR / "chau_civic_event_query_attendance_by_category.sql")

        # ----------------------------------------------------
        # STEP 4: RUN KPI QUERY (ACTION-DRIVEN)
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "chau_civic_event_query_kpi_attendance.sql")
